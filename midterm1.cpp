...
public:
    ...
    ~DoublyLinkedList() // destructor for DoublyLinkedList object
    {
        while (head) // while a head exists
        {
            Node *temp = head; // make pointer to address of head
            head = head->next; // move head pointer to next
            delete temp;       // delete current node
        }
    }

    void print() // function to print all data of nodes from DLL
    {
        Node *current = head; // make pointer to head node
        if (!current)         // if there's no Nodes
        {
            cout << "List is empty." << endl; // error message
            return;                           // end
        }
        ...
        {
            cout << current->data << " "; // output the node's data (public because struct)
            current = current->next;      // move pointer of current to next
        }
        cout << endl; // formatting
    }

    void print_reverse() // function to print all data of nodes from DLL from tail to head
    {
        Node *current = tail; // make current pointer tail instead
        if (!current)         // if there's no Nodes
        {
            cout << "List is empty." << endl; // error message
            return;                           // end
        ...
        {
            cout << current->data << " "; // output the node's data (public because struct)
            current = current->prev;      // move pointer of current to previous
        }
        cout << endl; // formatting
    }
...
int main() // main, where things are called from
{
